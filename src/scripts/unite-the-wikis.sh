...
# 		WIKI={{ wiki_id | quote }} {{ m_mediawiki | quote }}/maintenance/run importImages --search-recursively "$m_uploads_dir/$wiki/$dir"
# 	done
# done

# Seemed to be a memory leak in rebuildall.php. Breaking it up into its
# component scripts to limit impact.
# WIKI={{ wiki_id | quote }} {{ m_mediawiki | quote }}/maintenance/run rebuildall


# FIXME: Is this even necessary with elasticsearch?
echo "Beginning rebuildtextindex.php script"
WIKI={{ wiki_id | quote }} {{ m_mediawiki | quote }}/maintenance/run rebuildtextindex

echo "Beginning rebuildrecentchanges.php script"
WIKI={{ wiki_id | quote }} {{ m_mediawiki | quote }}/maintenance/run rebuildrecentchanges

# num_pages=$(WIKI={{ wiki_id | quote }} {{ m_mediawiki | quote }}/maintenance/run showSiteStats.php | grep "Total pages" | sed 's/[^0-9]*//g')
...
while [ "$end_id" -lt "$num_pages" ]; do
	...
	WIKI={{ wiki_id | quote }} {{ m_mediawiki | quote }}/maintenance/run refreshLinks --e "$end_id" -- "$start_id"
done

start_id=$(($end_id + 1))
echo "Running final refreshLinks.php in case there are more pages beyond num_pages, beyond $start_id"
WIKI={{ wiki_id | quote }} {{ m_mediawiki | quote }}/maintenance/run refreshLinks "$start_id"
